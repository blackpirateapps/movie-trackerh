...
- **Android Keystore Signing via Repository Secrets**: The CI workflow passes `KEYSTORE_BASE64` (or `ANDROID_KEYSTORE_BASE64`) as environment variables to bash, decodes the keystore into `upload-keystore.jks`, and dynamically generates `key.properties` when secrets (`KEYSTORE_PASSWORD`, `KEY_ALIAS`, `KEY_PASSWORD`) are present, falling back to debug signing when omitted.
